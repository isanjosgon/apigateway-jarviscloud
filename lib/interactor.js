  ...
  fetchcurrency () {
    ...
    new Currency(self.params)
      ...
      .then(function (res) {
        ...
        self.cache.write(weatherkey({ from:self.params.from,to:self.params.to }),res);
      })
      .catch(function (err,status) {
        if (status && status == 400) {
          return self.presenter.ko(err,status);
        }
        self.cache.read(weatherkey({ from:self.params.from,to:self.params.to }),function (err,res) {
          if (err || !res) {
            return self.presenter.ko('Impossible connect to currency service.');
          }
          self.presenter.ok({
            version:config.version,
            from:self.params.from,
